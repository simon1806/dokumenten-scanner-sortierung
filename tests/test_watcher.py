...
class WatcherTests(unittest.TestCase):
    ...
    def test_heartbeat_logs_runtime_folder_state_queue_and_last_result_every_ten_minutes(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("schema=2", heartbeat)
            self.assertIn("ereignis=monitor_heartbeat", heartbeat)
            self.assertRegex(heartbeat, r"sitzung=[0-9a-f]{32}")
            ...
            self.assertIn("intervall_s=600.0", heartbeat)
